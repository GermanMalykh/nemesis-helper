...
export class MonsterTokensService<MonsterTokenType extends MonsterTokenBase> {
    ...
    public initMonsters(defaultMonsterBagConfig: Record<MonsterType, number>, availableMonsters: MonsterTokenType[], playerCount: number): void {
    ...
    public loadBags(monsters: { availableMonsters: MonsterTokenType[]; activeMonsters: MonsterTokenType[]; bagMonsters: MonsterTokenType[] }): void {
    ...
    private generateMonsterTypeArray(defaultMonsterBagConfig: Record<MonsterType, number>, type: MonsterType, additional: number = 0): MonsterType[] {
        const length: number = defaultMonsterBagConfig[type] + additional;
        return Array.from({ length }, () => type);
    }

    private transferMonster(from: MonsterSource, to: MonsterSource, predicate?: (monster: MonsterTokenType) => boolean): MonsterTokenType | undefined {
